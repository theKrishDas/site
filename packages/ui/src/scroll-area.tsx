...
function Viewport({ className, ...props }: BUIScrollArea.Viewport.Props) {
  return (
    <BUIScrollArea.Viewport
      className={`--ui-scroll-area-viewport ui:corner-squircle ui:-outline-offset-1 ui:h-full ui:overscroll-contain ui:rounded-2xl ui:focus-visible:outline-2 ui:focus-visible:outline-ios-blue ${className}`}
      {...props}
    />
  )
}
